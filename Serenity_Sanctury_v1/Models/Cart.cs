...
using Serenity_Sanctury_v1.Models.DTOs;
using Serenity_Sanctury_v1.Models.ExtensionMethods; // Assuming you have DTOs
...
namespace Serenity_Sanctury_v1.Models
{
    public class Cart
    {
        ...
        public void Load(Func<int, Product?> getProductById)
        {
            // get cart items from session and cookie
            items = session.GetObject<List<CartItem>>(CartKey)
                ?? new List<CartItem>();
            ...
            if (cookieItems.Count > items.Count)
            {
                ...
                foreach (CartItemDTO storedItem in cookieItems)
                {
                    var product = getProductById(storedItem.ProductID);

                    // Skip if product is null - it's no longer in the database
                    if (product != null)
                    {
                        CartItem item = new CartItem
                        {
        ...
        public CartItem? GetById(int? id)
        {
            ...
            {
                return items.FirstOrDefault(ci => ci.Product?.ProductID == id);
            }
        }
